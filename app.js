...
showPage = function(id) {
    ...
    try {
        const res = await fetch(`/api/hr/payrolls/${id}`, { method: 'DELETE' });
        if (!res.ok) throw new Error('Error eliminando planilla');
        showToast('✅ Planilla eliminada', 'success');
        loadPayrolls();
    } catch (e) {
        showToast('❌ Error: ' + e.message, 'error');
    }
...
/* ========== MÓDULOS ADICIONALES DE RRHH ========== */

/* ---- CONTRATOS ---- */
let currentContract = null;

async function showContractsSection() {
    document.getElementById('hrApplicationsSection').style.display = 'none';
    document.getElementById('hrPayrollSection').style.display = 'none';
    document.getElementById('hrContractsSection').style.display = 'block';
    document.getElementById('hrConfidentialitySection').style.display = 'none';
    document.getElementById('hrDocumentsSection').style.display = 'none';
    document.getElementById('hrPermissionsSection').style.display = 'none';
    document.getElementById('hrCertificatesSection').style.display = 'none';
    await loadContracts();
}
...
function renderContractsList(contracts) {
    ...
    if (!contracts.length) {
        tbody.innerHTML = '<tr><td colspan="6" style="text-align:center;color:var(--text-muted);">No hay contratos registrados</td></tr>';
        return;
    }
    tbody.innerHTML = contracts.map(c => `
        <tr>
            ...
            <td>${new Date(c.startDate).toLocaleDateString('es-SV')}</td>
            <td><span class="status-badge status-${c.status}">${c.status}</span></td>
            <td>
                <button class="btn-icon" onclick="viewContract('${c.id}')" title="Ver">👁️</button>
                <button class="btn-icon" onclick="downloadContractPDF('${c.id}')" title="Descargar PDF">📄</button>
            </td>
        </tr>
    `).join('');
}
...
function showNewContractForm() {
    document.getElementById('contractFormPanel').style.display = 'block';
...
async function saveContract() {
    ...
    const contract = {
        ...
        position: document.getElementById('contractPosition').value,
        salary: parseFloat(document.getElementById('contractSalary').value),
        startDate: document.getElementById('contractStartDate').value,
        contractType: document.getElementById('contractType').value,
        workSchedule: document.getElementById('contractSchedule').value,
    ...
    try {
        ...
        document.getElementById('contractFormPanel').style.display = 'none';
        loadContracts();
    } catch (e) {
        showToast('❌ Error: ' + e.message, 'error');
    }
...
function downloadContractPDF(id) {
    ...
    // Aquí se implementaría la generación del PDF en el cliente
    // Por ahora solo mostramos un mensaje
    setTimeout(() => {
        showToast('✅ PDF generado (funcionalidad pendiente)', 'success');
    }, 1000);
}
...
async function showConfidentialitySection() {
    document.getElementById('hrApplicationsSection').style.display = 'none';
    document.getElementById('hrPayrollSection').style.display = 'none';
    document.getElementById('hrContractsSection').style.display = 'none';
    document.getElementById('hrConfidentialitySection').style.display = 'block';
    document.getElementById('hrDocumentsSection').style.display = 'none';
    document.getElementById('hrPermissionsSection').style.display = 'none';
    document.getElementById('hrCertificatesSection').style.display = 'none';
    await loadConfidentialityLetters();
}

async function loadConfidentialityLetters() {
    try {
        const res = await fetch('/api/hr/confidentiality');
        if (!res.ok) throw new Error('Error cargando cartas');
        const letters = await res.json();
        renderConfidentialityList(letters);
    } catch (e) {
        showToast('❌ Error: ' + e.message, 'error');
    }
}

function renderConfidentialityList(letters) {
    const tbody = document.getElementById('confidentialityTableBody');
    if (!letters.length) {
        tbody.innerHTML = '<tr><td colspan="5" style="text-align:center;color:var(--text-muted);">No hay cartas registradas</td></tr>';
        return;
    }
    tbody.innerHTML = letters.map(l => `
        <tr>
            <td>${l.empName}</td>
            <td>${l.position}</td>
            <td>${new Date(l.createdAt).toLocaleDateString('es-SV')}</td>
            <td><span class="status-badge status-${l.status}">${l.status}</span></td>
            <td>
                <button class="btn-icon" onclick="viewConfidentialityLetter('${l.id}')" title="Ver">👁️</button>
                <button class="btn-icon" onclick="downloadConfidentialityPDF('${l.id}')" title="Descargar PDF">📄</button>
            </td>
        </tr>
    `).join('');
}

function showNewConfidentialityForm() {
    document.getElementById('confidentialityFormPanel').style.display = 'block';
    document.getElementById('confidentialityForm').reset();
}

async function saveConfidentialityLetter() {
    ...
    const letter = {
        empId,
        empName: `${employee.firstName} ${employee.lastName}`,
        position: document.getElementById('confPosition').value,
        terms: document.getElementById('confTerms').value
    };
    ...
    try {
        ...
        document.getElementById('confidentialityFormPanel').style.display = 'none';
        loadConfidentialityLetters();
    } catch (e) {
        showToast('❌ Error: ' + e.message, 'error');
    }
}

function downloadConfidentialityPDF(id) {
    showToast('📄 Generando PDF de la carta...', 'info');
    setTimeout(() => {
        showToast('✅ PDF generado (funcionalidad pendiente)', 'success');
    }, 1000);
}
...
async function showDocumentsSection() {
    document.getElementById('hrApplicationsSection').style.display = 'none';
    document.getElementById('hrPayrollSection').style.display = 'none';
    document.getElementById('hrContractsSection').style.display = 'none';
    document.getElementById('hrConfidentialitySection').style.display = 'none';
    document.getElementById('hrDocumentsSection').style.display = 'block';
    document.getElementById('hrPermissionsSection').style.display = 'none';
    document.getElementById('hrCertificatesSection').style.display = 'none';
    await loadDocuments();
}
...
function renderDocumentsList(documents) {
    ...
    if (!documents.length) {
        tbody.innerHTML = '<tr><td colspan="6" style="text-align:center;color:var(--text-muted);">No hay documentos subidos</td></tr>';
        return;
    }
    ...
    tbody.innerHTML = documents.map(d => `
        <tr>
            <td>${d.empName}</td>
            <td>${typeLabels[d.documentType] || d.documentType}</td>
            <td>${d.fileName}</td>
            <td>${(d.fileSize / 1024).toFixed(2)} KB</td>
            <td>${new Date(d.uploadedAt).toLocaleDateString('es-SV')}</td>
            <td>
                <button class="btn-icon" onclick="viewDocument('${d.filePath}')" title="Ver">👁️</button>
                <button class="btn-icon" onclick="downloadDocument('${d.filePath}', '${d.fileName}')" title="Descargar">⬇️</button>
                <button class="btn-icon" onclick="deleteDocument('${d.id}')" title="Eliminar">🗑️</button>
            </td>
        </tr>
    `).join('');
}
...
function showUploadDocumentForm() {
    document.getElementById('documentUploadPanel').style.display = 'block';
...
async function uploadDocument() {
    ...
    const empId = document.getElementById('docEmpId').value;
    const employee = state.employees.find(e => e.id === empId);
    if (!employee) {
        showToast('❌ Empleado no encontrado', 'error');
        return;
    ...
    formData.append('empName', `${employee.firstName} ${employee.lastName}`);
    ...
    try {
        ...
        document.getElementById('documentUploadPanel').style.display = 'none';
        loadDocuments();
    } catch (e) {
        showToast('❌ Error: ' + e.message, 'error');
    }
...
async function showPermissionsSection() {
    ...
    document.getElementById('hrConfidentialitySection').style.display = 'none';
    document.getElementById('hrDocumentsSection').style.display = 'none';
    document.getElementById('hrPermissionsSection').style.display = 'block';
    document.getElementById('hrCertificatesSection').style.display = 'none';
...
function renderPermissionsList(permissions) {
    ...
    if (!permissions.length) {
        tbody.innerHTML = '<tr><td colspan="7" style="text-align:center;color:var(--text-muted);">No hay solicitudes de permiso</td></tr>';
        return;
    }
    ...
    tbody.innerHTML = permissions.map(p => `
        <tr>
            <td>${p.empName}</td>
            <td>${typeLabels[p.permissionType] || p.permissionType}</td>
            <td>${new Date(p.startDate).toLocaleDateString('es-SV')}</td>
            <td>${new Date(p.endDate).toLocaleDateString('es-SV')}</td>
            <td>${p.reason}</td>
            <td><span class="status-badge status-${p.status}">${p.status}</span></td>
            <td>
                ${p.status === 'pending' ? `
                    <button class="btn-icon" onclick="approvePermission('${p.id}')" title="Aprobar">✅</button>
                    <button class="btn-icon" onclick="rejectPermission('${p.id}')" title="Rechazar">❌</button>
                ` : ''}
                <button class="btn-icon" onclick="deletePermission('${p.id}')" title="Eliminar">🗑️</button>
            </td>
        </tr>
    `).join('');
}
...
function showNewPermissionForm() {
    document.getElementById('permissionFormPanel').style.display = 'block';
...
async function savePermission() {
    ...
    const permission = {
        empId,
        empName: `${employee.firstName} ${employee.lastName}`,
        permissionType: document.getElementById('permType').value,
    ...
    try {
        ...
        if (!res.ok) throw new Error('Error guardando permiso');
        showToast('✅ Permiso guardado', 'success');
        document.getElementById('permissionFormPanel').style.display = 'none';
        loadPermissions();
    } catch (e) {
        showToast('❌ Error: ' + e.message, 'error');
    }
...
async function approvePermission(id) {
    try {
        const res = await fetch(`/api/hr/permissions/${id}`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ status: 'approved', approvedDate: new Date().toISOString() })
        });
        if (!res.ok) throw new Error('Error aprobando permiso');
...
async function rejectPermission(id) {
    ...
    try {
        ...
        if (!res.ok) throw new Error('Error rechazando permiso');
        showToast('✅ Permiso rechazado', 'success');
        loadPermissions();
    } catch (e) {
        showToast('❌ Error: ' + e.message, 'error');
    }
...
async function deletePermission(id) {
    if (!confirm('¿Estás seguro de eliminar esta solicitud?')) return;
    try {
        const res = await fetch(`/api/hr/permissions/${id}`, { method: 'DELETE' });
        if (!res.ok) throw new Error('Error eliminando permiso');
...
/* ---- CONSTANCIAS DE TIEMPO LABORAL ---- */
async function showCertificatesSection() {
    document.getElementById('hrApplicationsSection').style.display = 'none';
    document.getElementById('hrPayrollSection').style.display = 'none';
    document.getElementById('hrContractsSection').style.display = 'none';
    document.getElementById('hrConfidentialitySection').style.display = 'none';
    document.getElementById('hrDocumentsSection').style.display = 'none';
    document.getElementById('hrPermissionsSection').style.display = 'none';
    document.getElementById('hrCertificatesSection').style.display = 'block';
...
async function loadCertificates() {
    try {
        ...
        if (!res.ok) throw new Error('Error cargando constancias');
...
function renderCertificatesList(certificates) {
    ...
    if (!certificates.length) {
        tbody.innerHTML = '<tr><td colspan="6" style="text-align:center;color:var(--text-muted);">No hay constancias generadas</td></tr>';
        return;
    }
    tbody.innerHTML = certificates.map(c => `
        <tr>
            ...
            <td>${c.position}</td>
            <td>${c.timeWorked}</td>
            <td>${c.includeSalary ? `$${c.salary.toFixed(2)}` : 'No incluido'}</td>
            <td>${new Date(c.generatedAt).toLocaleDateString('es-SV')}</td>
            <td>
                <button class="btn-icon" onclick="downloadCertificatePDF('${c.id}')" title="Descargar PDF">📄</button>
            </td>
        </tr>
    `).join('');
}

function showGenerateCertificateForm() {
    document.getElementById('certificateFormPanel').style.display = 'block';
...
async function generateCertificate() {
    ...
    const data = {
        empId,
        includeSalary: document.getElementById('certIncludeSalary').checked,
        purpose: document.getElementById('certPurpose').value
    };

    try {
        const res = await fetch('/api/hr/certificates/generate', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
        ...
        document.getElementById('certificateFormPanel').style.display = 'none';
        loadCertificates();
        
        // Mostrar preview
        showCertificatePreview(result.certificate);
    } catch (e) {
        showToast('❌ Error: ' + e.message, 'error');
    }
}
...
function showCertificatePreview(cert) {
    const html = `
        <div class="certificate-preview">
            <h2 style="text-align:center;margin-bottom:30px;">CONSTANCIA DE TIEMPO LABORAL</h2>
            <p style="text-align:justify;line-height:1.8;">
                Por medio de la presente, se hace constar que <strong>${cert.empName}</strong>, 
                se desempeña en esta empresa en el cargo de <strong>${cert.position}</strong>, 
                desde el <strong>${new Date(cert.startDate).toLocaleDateString('es-SV')}</strong>, 
                con un tiempo laborado de <strong>${cert.timeWorked}</strong>.
            </p>
            ${cert.includeSalary ? `
                <p style="text-align:justify;line-height:1.8;">
                    Actualmente devenga un salario mensual de <strong>$${cert.salary.toFixed(2)}</strong>.
                </p>
            ` : ''}
            <p style="text-align:justify;line-height:1.8;">
                Se extiende la presente constancia a solicitud del interesado 
                ${cert.purpose ? `para <strong>${cert.purpose}</strong>` : ''}.
            </p>
            <div style="margin-top:60px;text-align:center;">
                <p>_______________________________</p>
                <p><strong>Firma y Sello</strong></p>
                <p>${state.adminConfig.company}</p>
            </div>
        </div>
    `;
    ...
    modal.innerHTML = `
        <div class="modal-content" style="max-width:800px;">
            ...
            <div class="modal-footer">
                ...
                <button class="btn btn-primary" onclick="downloadCertificatePDF('${cert.id}')">Descargar PDF</button>
            </div>
        </div>
    `;
...
function downloadCertificatePDF(id) {
    showToast('📄 Generando PDF de la constancia...', 'info');
    setTimeout(() => {
        showToast('✅ PDF generado (funcionalidad pendiente)', 'success');
    }, 1000);
}
